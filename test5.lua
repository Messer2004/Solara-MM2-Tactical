...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    local args = {...}
    if State.Aim and method == "FireServer" and self.Name == "ShootGun" then
...
RS.RenderStepped:Connect(function()
    ...
    if Plr.Character and Plr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if State.Helicopter then
            ...
            if tor then
                for _, joint in pairs(Plr.Character:GetDescendants()) do
                    if joint:IsA("Motor6D") and joint.Name ~= "RootJoint" then
                        joint.C0 = joint.C0 * CFrame.Angles(math.rad(45), math.rad(45), math.rad(45))
                    end
                end
                tor.CFrame = tor.CFrame * CFrame.Angles(0, math.rad(50), 0)
            end
        end
        ...
        elseif not State.Aura and not State.Helicopter then 
...
Main:CreateToggle({Name = "Silent Aim (On Murderer)", Callback = function(v) State.Aim = v end})
...
Fling:CreateToggle({Name = "Вертолёт (Helicopter)", Callback = function(v) State.Helicopter = v end})
