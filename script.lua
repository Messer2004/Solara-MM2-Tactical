...
RS.RenderStepped:Connect(function()
    Circle.Visible, Circle.Radius, Circle.Position = showFOV, fFOV, Cam.ViewportSize / 2
end)
...
local function getRolePlr(r)
    for _, p in pcall(pairs, Plrs:GetPlayers()) do
...
task.spawn(function()
    while task.wait() do
        ...
        if (fly or flActive or flAura) and chr then
            for _, p in pairs(chr:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide = false end end
        end
        if flAura and hrp and chr:FindFirstChildOfClass("Humanoid") and chr:FindFirstChildOfClass("Humanoid").Health > 0 then
            hrp.Velocity, hrp.RotVelocity = Cam.CFrame.LookVector * 150, Vector3.new(0, 15000, 0)
        end
    end
end)
...
task.spawn(function()
    while task.wait(0.3) do
        ...
        if pickup and hrp and drop and drop:IsA("BasePart") then
            local old = hrp.CFrame hrp.CFrame = drop.CFrame + Vector3.new(0,1,0) task.wait(0.1) hrp.CFrame = old
        end
    end
end)
...
local function toggleESP(v)
    esp = v
    if esp then
        RS:BindToRenderStep("MUpdate", 200, function()
            for _, p in pairs(Plrs:GetPlayers()) do
                if p ~= LP and p.Character then
                    local h = p.Character:FindFirstChild("MesserHighlight") or Instance.new("Highlight", p.Character)
                    h.Name, h.FillTransparency, h.OutlineTransparency, h.FillColor, h.OutlineColor = "MesserHighlight", 0.5, 0, getRole(p), h.FillColor
                end
            end
        end)
    else
        RS:UnbindFromRenderStep("MUpdate")
        for _, p in pairs(Plrs:GetPlayers()) do if p.Character and p.Character:FindFirstChild("MesserHighlight") then p.Character.MesserHighlight:Destroy() end end
    end
end
...
local function toggleFly(v)
    ...
    if fly and r and h then
        local bv = Instance.new("BodyVelocity", r) bv.Name, bv.MaxForce, bv.Velocity = "MesserFlyBV", Vector3.new(1,1,1)*math.huge, Vector3.new(0,0,0)
        fConn = RS.Heartbeat:Connect(function()
            if r and h and fly then bv.Velocity = h.MoveDirection.Magnitude > 0 and Cam.CFrame.LookVector * 45 or Vector3.new(0,0,0) end
        end)
    else
        if fConn then fConn:Disconnect() end if r and r:FindFirstChild("MesserFlyBV") then r.MesserFlyBV:Destroy() end
    end
end

local function toggleSprint(v) sprint = v local h = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid") if h then h.WalkSpeed = sprint and walkSpeedSprint or walkSpeedNormal end end
...
T1:CreateSlider({Name = "WalkSpeed Customizer", Range = {16, 120}, Increment = 1, CurrentValue = 16, Callback = function(v) walkSpeedNormal = v if not sprint and LP.Character and LP.Character:FindFirstChildOfClass("Humanoid") then LP.Character.Humanoid.WalkSpeed = v end end})
...
T1:CreateSlider({Name = "Aim FOV Radius", Range = {30, 500}, Increment = 5, CurrentValue = 150, Callback = function(v) fFOV = v end})
...
T3:CreateButton({Name = "Fling Selected Target", Callback = function() if selPlr ~= "" then fling(Plrs:FindFirstChild(selPlr)) end end})
T3:CreateButton({Name = "Fling Murderer", Callback = function() fling(getRolePlr("m")) end})

UIS.InputBegan:Connect(function(input, gp)
    if gp then return end
    if input.KeyCode == Enum.KeyCode.V then toggle1:Set(not esp)
    elseif input.KeyCode == Enum.KeyCode.X then toggle2:Set(not fly)
    elseif input.KeyCode == Enum.KeyCode.Z then toggle3:Set(not sprint) end
end)
