...
-- Основной цикл физики для NoClip, Anti-Fling и самого Fling механика
task.spawn(function()
    while task.wait() do
        ...
        -- Сама механика закрутки хитбокса (Fling)
...
local function ActionFling(TargetPlayer)
    ...
    -- Активируем закрутку, летим к цели, разносим физику и возвращаемся
    FlingActive = true
...
-- Функция генерации списка игроков для Dropdown меню
...
-- Кнопка обновления списка игроков в меню
...
FlingSection:CreateButton({
    ...
    Callback = function()
        if SelectedPlayerName ~= "" then
            ...
            Rayfield:Notify({Title = "Внимание", Content = "Сначала выберите ник из списка!", Duration = 2})
        end
    end,
})
...
FlingSection:CreateButton({
    ...
    Callback = function()
        ...
        if target then ActionFling(target) else Rayfield:Notify({Title = "Ряды чисты", Content = "Мардер не обнаружен", Duration = 2}) end
    end,
})
...
FlingSection:CreateButton({
    ...
    Callback = function()
        ...
        if target then ActionFling(target) else Rayfield:Notify({Title = "Ряды чисты", Content = "Шериф не обнаружен", Duration = 2}) end
    end,
})

-- Возвращаемся к стандартным кнопкам основного меню
Tab:CreateToggle({
...
Rayfield:Notify({
    Title = "Executed",
    Content = "Messer PC Build Updated with Fling System",
    Duration = 3,
    Image = 4483362458,
})
